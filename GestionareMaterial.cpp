...
void __fastcall TFGestionareMaterial::Image4Click(TObject *Sender)
{
	if(Edit8->Text!="")
	{
        ...
        else
		{
			dm->QCautareMaterial->Close();
			dm->QCautareMaterial->SQL->Clear();
			dm->QCautareMaterial->SQL->Add("SELECT * FROM MATERIAL WHERE PRET=:PRET");
			dm->QCautareMaterial->ParamByName("PRET")->AsString = Edit8->Text;
			dm->QCautareMaterial->Open();
		}
	}
...
void __fastcall TFGestionareMaterial::SpeedButton7Click(TObject *Sender)
{
	...
	else
	{
		dm->QLiber->Close();
		dm->QLiber->SQL->Clear();
		dm->QLiber->SQL->Add("DELETE FROM MATERIAL WHERE PRET=:PRET");
...
void __fastcall TFGestionareMaterial::RB3Click(TObject *Sender)
{
	Edit8->Clear();
    Edit8->TextHint = "Pret";
}
